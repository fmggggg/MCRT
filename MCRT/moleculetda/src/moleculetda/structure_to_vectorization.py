...
def coords_to_pd(
    coords, periodic: bool = False, weights = None,return_persistence = False
):
    if return_persistence:
        dgms,m=construct_pds(coords, periodic=periodic, weights=weights,return_persistence =return_persistence)
        arr_dgms = diagrams_to_arrays(dgms)  # convert to array representations
        return arr_dgms,m
    else:
        dgms = construct_pds(coords, periodic=periodic, weights=weights,return_persistence =return_persistence)
        arr_dgms = diagrams_to_arrays(dgms)  # convert to array representations
        return arr_dgms
