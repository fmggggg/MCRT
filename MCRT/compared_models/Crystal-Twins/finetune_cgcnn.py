...
class FineTune(object):
    ...
    def train(self):
        ...
        print(f"Validation loader length: {len(self.valid_loader)}")
        print(f"Test loader length: {len(self.test_loader)}")
        print("Starting the training loop...")
        for epoch_counter in range(self.config['epochs']):
            print(f"Epoch {epoch_counter} start...")
...
# from sklearn.metrics import r2_score
    ...
    def test(self):
        ...
        if self.config['task'] == 'regression':
            mae_errors = AverageMeter()
            r2_scores = AverageMeter()  # Add R2 score tracker
        else:
            accuracies = AverageMeter()
            precisions = AverageMeter()
            recalls = AverageMeter()
        ...
        with torch.no_grad():
            ...
            if self.config['task'] == 'regression':
                ...
                r2_scores.update(r2, len(test_targets))
                print('Test: [{0}/{1}], '
                    'Loss {loss.val:.4f} ({loss.avg:.4f}), '
                    'MAE {mae_errors.val:.3f} ({mae_errors.avg:.3f}), '
                    'R2 {r2_scores.val:.3f} ({r2_scores.avg:.3f})'.format(
                    bn, len(self.test_loader), loss=losses,
                    mae_errors=mae_errors, r2_scores=r2_scores))
            else:
                print('Test: [{0}/{1}], '
                    'Loss {loss.val:.4f} ({loss.avg:.4f}), '
                    'Accu {accu.val:.3f} ({accu.avg:.3f}), '
        ...
        if self.config['task'] == 'regression':
            print('MAE {mae_errors.avg:.3f}'.format(mae_errors=mae_errors))
            print('R2 {r2_scores.avg:.3f}'.format(r2_scores=r2_scores))
            return losses.avg, mae_errors.avg, r2_scores.avg
        else:
            print('AUC {auc.avg:.3f}'.format(auc=auc_scores))
            return losses.avg, auc_scores.avg
...
if __name__ == "__main__":
    ...
    if 'band' in config['dataset']['root_dir']:
        ...
        task_name = os.path.basename(config['dataset']['root_dir'])

    fine_tune = FineTune(config)
    fine_tune.train()
    loss, mae, r2 = fine_tune.test()

    import pandas as pd
    ftf = os.path.basename(config['fine_tune_from'])
    seed = config['random_seed']
    fn = '{}_{}_200epochs_{}.csv'.format(ftf, task_name, seed)
    print(fn)
    df = pd.DataFrame([[loss, mae, r2]])
    df.to_csv(
