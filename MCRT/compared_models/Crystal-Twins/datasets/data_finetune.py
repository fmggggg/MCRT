...
def get_train_val_test_loader(dataset, collate_fn=default_collate, batch_size=64, random_seed=2, val_ratio=0.1, test_ratio=0.1, return_test=False, num_workers=1, pin_memory=False, **kwargs):
    total_size = len(dataset)
    train_ratio = 1 - val_ratio - test_ratio
    indices = list(range(total_size))
    print("The random seed is: ", random_seed)
    ...
    print(f"Train indices: {train_indices[:10]} ... {train_indices[-10:]}")
    print(f"Validation indices: {val_indices[:10]} ... {val_indices[-10:]}")
    print(f"Test indices: {test_indices[:10]} ... {test_indices[-10:]}")
