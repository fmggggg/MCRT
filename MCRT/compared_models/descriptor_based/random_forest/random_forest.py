...
parser.add_argument('--descriptor-csv', type=str,default=r'D:\Projects\MyProjects\MCRT\MCRT\compared_models\descriptor_based\T2\T2_soap_descriptors.csv', help='(str) Path to the descriptor CSV file.')
parser.add_argument('--label-csv', type=str,default=r'D:\Projects\MyProjects\MCRT\MCRT\compared_models\descriptor_based\T2\T2_diffusity.csv', help='(str) Path to the label CSV file.')
...
parser.add_argument('--split', type=str, default=r'D:\Projects\MyProjects\MCRT\MCRT\cifs\Additional_Computational_Data\T2_Predicted_Structures\dataset_split.json', help='(str) Path to the JSON file with train/val/test split information')
...
def main():
    ...
    if args.load_model:
        ...
        # Plotting
        plot_path = os.path.splitext(args.results_csv)[0] + '_rf.png'
        x = np.array(y_pred)
        y = np.array(y_test)
        xy = np.vstack([x, y])
        ...
        for n_estimators in [10,50,100,500,1000]:
            ...
            if args.outdir:
                ...
                with open(os.path.join(args.outdir, f'rf_{n_estimators}.pickle'), 'wb') as f:
                    pickle.dump(model, f)
            print(f'finish {n_estimators} r2_score:{score} MAE:{mae}')  
            
            if score > best_score:
                best_score = score
                best_model = copy.deepcopy(model)
                best_n_estimators = n_estimators  
            print(f"Best R^2 Score: {best_score} with n_estimators: {best_n_estimators}")
    ...
    if args.output:
        with open(args.output, 'a') as f:
            f.write(f"Best R^2 Score: {best_score} with n_estimators: {best_n_estimators}\n")
